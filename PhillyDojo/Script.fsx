...
let lines = readFile @"C:\development\demos\PhillyDojo\PhillyDojo\trainingsample.csv"
...
let linesAsArrays = lines |> Array.map(fun s -> s.Split(',')) 
...
// </F# QUICK-STARTER> 
let linesWithoutHeader = linesAsArrays.[1 .. ]
...
let lineToInts (line : string[]) =
   line |> Array.map(fun s -> (int)s)

let linesAsInts = linesWithoutHeader |> Array.map(lineToInts)
...
// [ YOUR CODE GOES HERE! ]
let linesAsRecords = linesAsInts |> Array.map(fun L -> { Label = L.[0]; Pixels = L.[1 ..] })
...
let distanceForPoints (P1: int[]) (P2: int[]) =
    Array.map2 (dist) P1 P2
    |> Array.sum 
    |> (float)
    |> Math.Sqrt 
...
let classify (unknown:int[]) =
    ...
    let minimum = 
      linesAsRecords 
      |> Array.minBy(fun r ->  distanceForPoints r.Pixels unknown )
    minimum.Label
...
//let fileToRecords path = 
//  let lines = readFile path


let validation = readFile @"C:\development\demos\PhillyDojo\PhillyDojo\validationsample.csv"
